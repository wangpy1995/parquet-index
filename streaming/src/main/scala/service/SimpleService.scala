...
@Component("simpleWSService")
class SimpleService {

  def createDStreams[T](task: Task, tables: Seq[String], requiredFactors: Seq[String], results: ArrayBuffer[Row])(f: (T, Row) => Row) = tables.map(ssc.simpleTaskDStream(task, _, requiredFactors)(f))
