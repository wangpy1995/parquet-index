...
import org.apache.spark.sql.catalyst.expressions.{Ascending, Attribute, BoundReference, Expression, SortOrder, UnsafeProjection}
...
case class ParquetSourceScanExec(
  ...
  val needsUnsafeRowConversion: Boolean = if (relation.fileFormat.isInstanceOf[ParquetFileFormat]) {
  ...
  @transient
  private val pushedDownFilters = dataFilters.flatMap(DataSourceStrategy.translateFilter) ++ Seq(In("value",Array("a","b")))
